...
const contarVisitas = () => {
    ...
    if (contador == 1) {
        resp.innerHTML = "Bem-Vindo! Esta é sua primeira visita ao nosso site."
    } else {
        resp.innerHTML = `Que bom que você voltou! Esta é sua visita número ${contador} ao nosso site.`
    }
...
const trocarClube = () => {
    ...
    } else { // caso (selecionou "Nenhum")
        dvTitulo.className = "row" // tira a classe de cor de dvTitulo
        imClube.className = "d-none" // oculta a imagem
        imClube.alt = "" // limpa texto vazio
        localStorage.remove("clube") // remove variável do localStorage
    }
}
...
const verificarClube = () => {
    if (localStorage.getItem("clube")) { // se já tiver salvo algum clube
        ...
        // conforme o clube,marca um dos elementos do input radio
        if (clube == "Brasil") {
