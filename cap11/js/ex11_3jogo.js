...
window.addEventListener("load", () => {
    ...
    if (!localStorage.getItem("jogoPalavra")) {
        ...
        frm.inLetra.disabled = true; // desabilita inLetra e botôes
    ...
    const tam = palavras.length; //números de palavras cadastradas

    // gera um número entre 0 e tam-1 (pois arredonda pra baixo)
    ...
    let novaPalavra = ""; // para montar palavra exibida (com letra e "_")
...
frm.btVerDica.addEventListener("click", ()=> {
    // verfica se o joagdor já clicou anteriormente no botão
    ...
    const chances = Number(respChances.innerText) - 1; // diminue 1 em chances
...
frm.addEventListener("submit", e => {
    ...
    if (palavraSorteada.includes(letra)) {
        ...
        const chances = Number(respChances.innerText) -1; // diminue n° de chances
...
const verificarFim = () => {
    ...
    if (chances == 0) {
        ...
        concluirJogo();
    } else if (respMensagemFinal.className == palavraSorteada) {
...
// modifica o texto da dica e desabilita os botôes jogar
const concluirJogo = () => {
